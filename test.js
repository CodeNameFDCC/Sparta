...
function Particle(x, y) {
    ...
    this.update = function () {
        ...
        if (distance < maxDistance) {
            this.x -= forceDirectionX * force * this.density + sin;
            this.y -= forceDirectionY * force * this.density + cos;
        } else {
            // 파티클이 원래 위치로 돌아감
            if (this.x !== this.baseX) {
                let dx = this.x - this.baseX + sin;
                this.x -= dx / 10;
            }
            if (this.y !== this.baseY) {
                let dy = this.y - this.baseY + cos;
                this.y -= dy / 10;
            }
        }
    }
...
// 애니메이션 루프
function animate1() {
    if(currentPage!=1) {requestAnimationFrame(animate1);return;} 
